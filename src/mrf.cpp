...
inline double MRF::getSumOfVector(std::vector<double> vector) {
  double sum = 0.0;
  for (int i = 0; i < (int)vector.size(); i++) sum += vector[i];
  return sum;
}

inline std::vector<double> MRF::getHadamardProduct(
    std::vector<double> vector1, std::vector<double> vector2) {
  for (int i = 0; i < (int)vector1.size(); i++) vector1[i] *= vector2[i];
  return vector1;
}

inline std::vector<double> MRF::normalizeVector(std::vector<double> vector) {
  double sum = getSumOfVector(vector);
  for (int i = 0; i < (int)vector.size(); i++) vector[i] /= sum;
  return vector;
}

inline double MRF::getEuclideanNormOfDiffVectors(std::vector<double> vector1,
...
inline std::vector<double> MRF::calculateTransitionMessage(
...
std::vector<std::vector<double>> MRF::getLikelihoodVectors(
    std::vector<double> validResidualErrors) {
...
std::vector<std::vector<double>> MRF::estimateMeasurementClassProbabilities(
    std::vector<std::vector<double>> likelihoodVectors) {
  ...
  for (int i = 0; i < maxLPBComputationNum_; i++) {
    ...
    double diffNorm = getEuclideanNormOfDiffVectors(
        measurementClassProbabilities[idx2], measurementClassProbabilitiesPrev);
    variation += diffNorm;
...
double MRF::predictFailureProbabilityBySampling(
    std::vector<std::vector<double>> measurementClassProbabilities) {
  int failureCnt = 0;
  for (int i = 0; i < samplingNum_; i++) {
    ...
    for (int j = 0; j < measurementNum; j++) {
      ...
      if (darts > validProb) continue;
      validMeasurementNum++;
      if (darts > measurementClassProbabilities[j][ALIGNED]) misalignedNum++;
    }
    double misalignmentRatio =
        (double)misalignedNum / (double)validMeasurementNum;
    double unknownRatio =
        (double)(measurementNum - validMeasurementNum) / (double)measurementNum;
...
void MRF::setAllMeasurementClassProbabilities(
...
void MRF::predictFailureProbability() {
  ...
  if (validResidualErrorsSize <= minValidResidualErrorsNum_) {
    std::cerr << "WARNING: Number of validResidualErrors is less than the "
  ...
  result.data = failureProbability_;
