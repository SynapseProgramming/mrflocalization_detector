...
enum MeasurementClass { ALIGNED = 0, MISALIGNED = 1, UNKNOWN = 2 };
...
class MRF {
 private:
  ...
  inline double getEuclideanNormOfDiffVectors(std::vector<double> vector1,
                                              std::vector<double> vector2);
  inline std::vector<double> calculateTransitionMessage(
      std::vector<double> probs);
  std::vector<std::vector<double>> getLikelihoodVectors(
 ...
 public:
  MRF()
      : nh_("~"),
        ...
        samplingNum_(1000),
        misalignmentRatioThreshold_(0.5),
    ...
    nh_.param("publish_classified_scans", publishClassifiedScans_,
    ...
    nh_.param("residual_error_resolution", residualErrorReso_,
              residualErrorReso_);
    nh_.param("min_valid_residual_errors_num", minValidResidualErrorsNum_,
              minValidResidualErrorsNum_);
    nh_.param("max_residual_errors_num", maxResidualErrorsNum_,
              maxResidualErrorsNum_);
    nh_.param("max_lpb_computation_num", maxLPBComputationNum_,
              maxLPBComputationNum_);
    nh_.param("sampling_num", samplingNum_, samplingNum_);
    nh_.param("misalignment_ratio_threshold", misalignmentRatioThreshold_,
              misalignmentRatioThreshold_);
    nh_.param("unknown_ratio_threshold", unknownRatioThreshold_,
